...
testing_data <- gsub("( [[:punct:]]*(d|D)+)+$", "", testing_data)
testing_data <- gsub("[^a-zA-Z]*$", "", testing_data)
...
cleaningfn1 <- function(data){
  ...
  newdataset <<- gsub( " & ", " and ", newdataset)
...
saveRDS(testing_data_nolastword, "./testingdatanolastword6.rds")
saveRDS(testing_data, "./testingdata6.rds")

rm(blogs, twitter, news, blogs_sample, twitter_sample, news_sample, testing_data_nolastword, testing_data, testing_data_lastwordonly)
...
saveRDS(all_sample_bigram, "./bigram_data24.rds")
saveRDS(all_sample_trigram, "./trigram_data24.rds")
saveRDS(all_sample_quadgram, "./quadgram_data12.rds")
saveRDS(all_sample_quintgram, "./quintgram_data6.rds")
saveRDS(all_sample_sextgram, "./sextgram_data3.rds")
